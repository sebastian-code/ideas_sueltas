...
def validate_install():
    ...
    if not (
        python_version
        and int(python_version.split(".")[0]) == 2
        and int(python_version.split(".")[1]) >= 7
    ):
...
def setup_folders(install_path):
    ...
    if not os.path.exists(app):
        ...
        exec_in_shell(
            "cd %s && git clone --branch master https://github.com/webnotes/erpnext.git app"
            % install_path
        )
    ...
    if not os.path.exists(lib):
        ...
        exec_in_shell(
            "cd %s && git clone --branch master https://github.com/webnotes/wnframework.git lib"
            % install_path
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--no_install_prerequisites", default=False, action="store_true"
    )
...
def create_user(username, password):
    ...
    p = subprocess.Popen(
        "useradd -m -d /home/{username} -s {shell} {username}".format(
            username=username, shell=os.environ.get("SHELL")
        ).split()
    )
    p.wait()
    p = subprocess.Popen(
        "passwd {username}".format(username=username).split(), stdin=subprocess.PIPE
    )
...
def setup_cron(install_path):
    erpnext_cron_entries = [
        "*/3 * * * * cd %s && python2.7 lib/wnf.py --run_scheduler >> erpnext-sch.log 2>&1"
        % install_path,
        "0 */6 * * * cd %s && python2.7 lib/wnf.py --backup >> erpnext-backup.log 2>&1"
        % install_path,
    ]
...
if __name__ == "__main__":
    ...
    print "Login as {username} with password {password}".format(
