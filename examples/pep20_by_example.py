...
def store(measurements):
    ...
    db = create_engine("mysql://user:password@localhost/db?charset=utf8&use_unicode=1")
    db.echo = False
    metadata = sqlalchemy.MetaData(db)
    table = sqlalchemy.Table(
        "measurements",
...
def process(response):
    ...
    divs = selector(lx)
    if divs:
        yield Item(utils.lx_to_text(divs[0]))
...
class FactorialTests(unittest.TestCase):
    ...
    def test_long(self):
        self.assertEqual(factorial(30), 265252859812191058636308480000000L)
...
if __name__ == "__main__":
    ...
    try:
        ...
        p = subprocess.Popen(
            ("pygmentize", "-f", "latex", "-l", "python", "-O", "full", sys.argv[0]),
    ...
    try:
        p = subprocess.Popen(
            ("pdflatex",), stdin=subprocess.PIPE, stdout=errout, stderr=errout
        )
