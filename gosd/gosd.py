...
def osd(
    ...
    if fake_translucent_bg:
        ...
        # bottom-right
        draw_corner(
            270, width - corner_size, height - corner_size, -corner_size, -corner_size
        )
        # top-right
    ...
    if drop_shadow:
        bitmap.draw_layout(
            fg_gc, off_x + drop_shadow_distance, off_y + drop_shadow_distance, layout
        )
    if debug_frame:
...
def popup(text, bgcolor=None, fgcolor=None, fontdesc=None, use_markup=False):
    ...
    frame = gobject.new(gtk.Frame, shadow_type=gtk.SHADOW_ETCHED_OUT, visible=True)
    win.add(frame)
