...
def render_text_to_image(
    font, size, text, outfile, margins, fgcolor, bgcolor, pango_markup=False
):
    """
...
if __name__ == "__main__":
    ...
    parser = OptionParser(
        usage="%prog --font=FONT --size=SIZE --text=TEXT --output=OUTFILE",
        version="%prog 0.1",
    )
    parser.add_option(
        "-f",
        "--font",
        dest="font",
        help='The font family to use. Pango descriptions such as "Sans Bold" work.',
        metavar="FONT",
    )
    parser.add_option(
        "-s",
        ...
        type="int",
        dest="size",
        help="The font size to use (in pixels).",
        metavar="SIZE",
    )
    parser.add_option(
        "-t",
    ...
    parser.add_option(
        ...
        "--output",
        dest="outfile",
        help="The filename to save the resulting PNG image to.",
        metavar="FILE",
    )
    parser.add_option(
        "-m",
    ...
    parser.add_option(
        ...
        "--background-color",
        dest="bgcolor",
        help="Background color to use, e.g. #333.",
        metavar="COLOR",
    )
    ...
    if not options.font:
        parser.error("No font family specified")
    if not options.size:
        parser.error("No font size specified")
    ...
    if options.fgcolor:
        try:
            fgcolor = str_to_rgba(options.fgcolor)
        except (ValueError):
            parser.error("Invalid foreground color")
    if options.bgcolor:
        try:
            bgcolor = str_to_rgba(options.bgcolor)
        except (ValueError):
            parser.error("Invalid background color")
