...
def daily_build_ubuntu_tweak(app="ubuntu-tweak", *args, **kwargs):
    ...
    version = local(
        "cd %s && python -c 'import %s;print %s.__version__'"
        % (src_dir, pkg_name, pkg_name),
        capture=True,
    )
    ...
    local(
        "cd %s && echo \"__version__ = '.'.join(map(str, VERSION)) + '+%s'\" >> %s/__init__.py"
        % (build_root, time.strftime("%Y%m%d", time.localtime()), pkg_name),
        capture=True,
    )
    ...
    for distro in distros.split("-"):
        changelog.make_daily(
            app,
            version,
            revno,
            distro,
            os.path.join(build_root, "debian/changelog"),
            suffix,
        )
        if mode == "b":
...
def release_ubuntu_tweak(app="ubuntu-tweak", version=None, *args, **kwargs):
    ...
    build_root = os.path.join(os.getcwd(), app)
    version = local(
        "cd %s && python -c 'import ubuntutweak;print ubuntutweak.__version__'"
        % src_dir,
        capture=True,
    )
