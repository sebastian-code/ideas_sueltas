...
class LyricsOSD:
    ...
    def LyricsShow(self, filename=None, elapsed=0):
        ...
        if self.__pos >= len(self.__lines):
            return
        ...
        print "%2d/%d %s %6.2f/%2.2f %6.2f %.2f %s" % (
            self.__pos,
            len(self.__lines),
            stime,
            elapsed,
...
class LyricsDBus:
    ...
    def __set_uri(self, uri):
        ...
        if uri is not None and uri != "":
            self.__uri = uri
            self.__shell.getSongProperties(
                uri,
    ...
    def __set_song_properties(self, prop):
        ...
        self.__lyfilename = "%s/.lyrics/%s/%s.lyric" % (
            os.getenv("HOME"),
            self.artist,
            self.title,
        )
    ...
    def connect(self):
        ...
        self.__handlers = [
            ...
            self.__player.connect_to_signal(
                "playingSongPropertyChanged", self.__property_changed
            ),
        ]

        self.__player.getPlaying(
            reply_handler=self.__set_playing, error_handler=self._report_dbus_error
        )

        self.__player.getElapsed(
            reply_handler=self.__set_elapsed, error_handler=self._report_dbus_error
        )

        self.__player.getPlayingUri(
            reply_handler=self.__set_uri, error_handler=self._report_dbus_error
        )
